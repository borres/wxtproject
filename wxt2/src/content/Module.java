...
public class Module extends DefaultMutableTreeNode
{
    ...
    private Vector <ScriptContent>m_ScriptContent;
    ...
    private Vector<Node> m_ModTocs;
        
    /** The last time we saved this module */
    private long m_lastTimeSaved;
    
    /** the primary sources we detect for this module*/
    private Vector<URI> m_sourceUris;
    ...
    private Vector<Node> m_foundRefLists;

    /** the author */
    private Fragment m_author;
    ...
    private Module(Scripthandler scriptH)
    {
        ...
        m_ScriptContent=new Vector<ScriptContent>(1);
        ...
        m_sourceUris=new Vector<URI>(10);
        m_referenceHolder=new ReferenceIndexHolder(m_Definitions);
        m_foundRefLists=new Vector<Node>();
    ...
    throws Exception
    {
        ...
        if(m_scriptElement.hasAttribute("author"))
        {
           ...
           m_author=m_Definitions.getFragments().get(auid);
        }
        // will allways have an author
        if(m_author==null)
           m_author=m_Definitions.getFragments().get(Definitions.DEFAULTAUTHOR);
        ...
        // move to build ?
        if(m_author!=null)
            m_author.addModuleToAuthor(this);
        ...
        if(m_scriptElement.hasChildNodes())
        {
            ...
            while(nod != null)
            {
                if(nod.getNodeType()==Node.ELEMENT_NODE)
                {
                    ...
                    {
                        try{m_ScriptContent.add(new ScriptContent(celt,this));}
                        catch(Exception e){m_Reporter.pushMessage("cannot_establish_xmlcontent");}
                    }
                    else if(celt.getNodeName().compareToIgnoreCase(TXTCONTENT)==0)
                    {
                        try{m_ScriptContent.add(new ScriptContent(celt,this));}
                        catch(Exception e){m_Reporter.pushMessage("cannot_establish_txtcontent");}                      
                    }
                    ...
                    else if(celt.getNodeName().compareToIgnoreCase("summary")==0)
                    {
                        try{
                            ...
                            if(celt.hasAttribute("location"))
                             ...
                             m_summary=new Fragment(celt,tmpuri,m_Definitions);
                       }
                        catch(Exception e)
                        {
        ...
        if(m_summary==null)
        {
            try{
                ...
                m_summary=new Fragment(m_description,this);
            }
            catch(Exception e)
            {
                m_Reporter.pushMessage("cannot_establish_summary",e.getMessage());
    ...
    {                 
        ...
        m_Producer.correctAddressing(m_Doc.getDocumentElement(), this, m_template.getabsoluteURI());
        ...
        Vector<Node> pilist=domer.getPIs(m_Doc, "_wxt"); 
        String piValue="";
        ...
        for(Node pi_node:pilist)
        {
            ...
            //------------------------------------
            // odtformulas content: <?_wxt formula  ....
            else if(cmd.getCommand().compareToIgnoreCase(PIcommand.FORMULA)==0)
            {
                DocumentFragment df=m_Producer.makePI_ODTFormula(this,cmd);
                pi_node.getParentNode().replaceChild(df, pi_node);
            }
            ...
            {
                DocumentFragment df=m_Producer.makePI_PopFragment(this,cmd);
                pi_node.getParentNode().replaceChild(df, pi_node);
            }
            ...
            else if(cmd.getCommand().compareToIgnoreCase(PIcommand.IXWORD)==0)
            {
                ...
                /*
                 m_scriptHandler.getIndexHolder().insertWord(this, cmd);
                
                // just remove the PI
                pi_node.getParentNode().removeChild(pi_node);
                 */
            }
            //----------------------------------------
            // <_wxt reftest
            ...
            {             
                if(cmd.paramExist(PIcommand.ID))
                {
                    ...
                    int globalindex=m_Definitions.getReferenceHolder().addReference(sid,this)+1;
                    // notify locally, and return index(-1 if not exist)
                    int localindex=m_referenceHolder.addReference(sid,this)+1;
            ...
            //----------------------------------
            // modulemenu: <?_wxt authorlist
            else if(cmd.getCommand().compareToIgnoreCase(PIcommand.AUTHORLIST)==0)
            {
                DocumentFragment df=m_Producer.makePI_ModulesFromAuthor(this, cmd);
                pi_node.getParentNode().replaceChild(df, pi_node);
            }
            ...
            {
                DocumentFragment df=m_Producer.makePI_SimpleAuthorList(this, cmd);
                pi_node.getParentNode().replaceChild(df, pi_node);
            }
            ...
            {
                DocumentFragment df=m_Producer.makePI_ExpandSimple(this, cmd);
                pi_node.getParentNode().replaceChild(df, pi_node);
            }
        ...
        //m_Reporter.pushMessage("stop_building", m_name);
    ...
    public void buildSecondPhase()
    {
        ...
        if(m_Doc==null)
        {
            ...
            //System.out.println("Have to rebuild module: "+m_name);
        }
        if(m_Doc==null)
       ...
       Vector<Node> pilist=domer.getPIs(m_Doc, "_wxt");      
        ...
        //m_Reporter.pushMessage("stop_building", m_id);
    ...
    public URI getAbsoluteUri()
    {
        ...
        Enumeration modules=this.depthFirstEnumeration();
     ...
     */
    public String getCatalog(){return accessutils.removeFilePart(m_absoluteUri).getPath();}

     /**
     * Returns the definitions for this module.
     * @return the definitions
     */
    public Definitions getDefinitions(){return m_Definitions;}
     ...
     */
    public ReferenceIndexHolder getReferenceHolder(){ return m_referenceHolder;}
     ...
     * @return the reporter
     */
    public reporter getReporter(){return m_Reporter;}
    
     /**
     * Returns the scripthandler controlling this module.
     * @return the Scripthandler
     */
    public Scripthandler getScriptHandler(){return m_scriptHandler;}
     ...
     * @return the Producer
     */
    public producer getProducer(){return m_Producer;}

    /**
     * Returns books as a commaseparated string.
     * @return m_books
     */
    public String getBooks(){return m_books;}
     ...
     */
    public boolean getLinkable(){return m_books.trim().indexOf(PIcommand.NEVER)==-1;}
     ...
     */
    public Vector<ScriptContent> getScriptContent(){return m_ScriptContent;}
     ...
     */
    public boolean getContainedInBook(String bookname){return m_books.indexOf(","+bookname+",")!=-1;}
     ...
     */
    public Content getRegisteredContent(String key){return m_PIContent.get(key);}
     ...
     */
    public Content registerContent(String key,Content cont){return m_PIContent.put(key, cont);}
     ...
     */
    public void clearSecondPhaseMark(){ m_doSecondPhase=false;}
         
    /**
     * Remember an url as a source for this module
     * @param theUri The URI we store
     */
    public void addDependency(URI theUri){m_sourceUris.add(theUri);}
    ...
    public String toString() {
        ...
        if (m_commandBefore == null  && m_commandAfter == null  ) s+=" none\n"; 
        else{
            if (m_commandBefore != null)
    ...
    /**
     * Produce a string suitablefor module inspection in a GUI
    ...
    public String Inspector()
    {
        ...
        sb.append("<tr>");
            sb.append("<td>Name</td>"+tdstartcol2+m_name+"</td>");
        sb.append("</tr>");
        
        sb.append("<tr>");
            sb.append("<td>Id</td>"+tdstartcol2+m_id+"</td>");
        sb.append("</tr>");
        
        sb.append("<tr>");
            sb.append("<td>Description</td>"+tdstartcol2+m_description+"</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_absoluteUri!=null)
                sb.append("<td>Location</td>"+tdstartcol2+m_absoluteUri.toString()+"</td>");
            else
                sb.append("<td>Location</td>"+tdstartcol2+"null</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_template!=null)
                sb.append("<td>Template</td>"+tdstartcol2+m_template.getabsoluteURI().toString()+"</td>");
            else
                sb.append("<td>Template</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_transformation!=null)
                sb.append("<td>Transformation</td>"+tdstartcol2+m_transformation.getabsoluteURI().toString()+"</td>");
            else
                sb.append("<td>Transformation</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_anchorCat!=null)
                sb.append("<td>Anchor</td>"+tdstartcol2+m_anchorCat.toString()+"</td>");
            else
                sb.append("<td>Anchor</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_books!=null)
                sb.append("<td>Books</td>"+tdstartcol2+m_books+"</td>");
            else
                sb.append("<td>Books</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_outputFormat!=null)
                sb.append("<td>Output Format</td>"+tdstartcol2+m_outputFormat+"</td>");
            else
                sb.append("<td>Output Format</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_commandBefore!=null)
                sb.append("<td>Run Before</td>"+tdstartcol2+m_commandBefore.getCommandLine()+"</td>");
            else
                sb.append("<td>Run Before</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");

        sb.append("<tr>");
            if(m_commandAfter!=null)
                sb.append("<td>Run After</td>"+tdstartcol2+m_commandAfter.getCommandLine()+"</td>");
            else
                sb.append("<td>Run After</td>"+tdstartcol2+"-</td>");
        sb.append("</tr>");
        ...
        sb.append("<tr>");
            sb.append("<td>Summary</td>"+tdstartcol2+getSummary().getFragmentAsString()+"</td>");
        sb.append("</tr>");
        ...
        {
            if(!( m_ScriptContent.elementAt(ix).getType().compareTo(Module.XMLCONTENT)==0))
                continue;
            String bup="";
            if(m_ScriptContent.elementAt(ix).usingBackup())
                bup="(backup)";
            sb.append("<tr>");
            sb.append("<td>XMLContent"+bup+"</td>"+tdstartcol2+m_ScriptContent.elementAt(ix).getAbsoluteEffectiveUri().toString()+"</td>");
            sb.append("</tr>");
        }
        for(int ix=0;ix < m_ScriptContent.size();ix++)
        {
            if(!( m_ScriptContent.elementAt(ix).getType().compareTo(Module.TXTCONTENT)==0))
                continue;
            String bup="";
            if(m_ScriptContent.elementAt(ix).usingBackup())
                bup="(backup)";
            sb.append("<tr>");
            sb.append("<td>TXTContent"+bup+"</td>"+tdstartcol2+m_ScriptContent.elementAt(ix).getAbsoluteEffectiveUri().toString()+"</td>");
            sb.append("</tr>");
        }
