...
public class Reference implements Comparable<Reference>
{
    ...
    */
    ...
    public Element produceIntextReference( Module m,PIcommand cmd, String style )
    {
     ...
     /**
     * Create and return an element for an referencelist
     ...
     */
    public Node produceListElement(Module m,String style, int index)
    {
        DocumentFragment df = m.getDoc().createDocumentFragment();
        Element mainDiv = m.getDoc().createElement("div");
        mainDiv.setAttribute("class", "refitem skiptranslate");

        if(style.compareTo(Definitions.HARVARD)==0)
        {
            // do harvard
            mainDiv.appendChild(m_propertMap.containsKey(AUTHORS_IN_LIST)?
                                mainDiv.appendChild(makePropertyElement(AUTHORS_IN_LIST,m)):
                                mainDiv.appendChild(makePropertyElement("title",m)));
            mainDiv.appendChild(makePropertyElement("year",m));
            if(m_propertMap.containsKey("chaptertitle")) mainDiv.appendChild(makePropertyElement("chaptertitle",m));
            if(m_propertMap.containsKey("journal")) mainDiv.appendChild(makePropertyElement("journal",m));
            if(m_propertMap.containsKey("editor")) mainDiv.appendChild(makePropertyElement("editor",m));
            if(!m_propertMap.containsKey(AUTHORS_IN_LIST))mainDiv.appendChild(makePropertyElement("title",m));
            if(m_propertMap.containsKey("place")) mainDiv.appendChild(makePropertyElement("place",m));
            if(m_propertMap.containsKey("publisher")) mainDiv.appendChild(makePropertyElement("publisher",m));
            if(m_propertMap.containsKey("pages")) mainDiv.appendChild(makePropertyElement("pages",m));
            if(m_propertMap.containsKey("isbn")) mainDiv.appendChild(makePropertyElement("isbn",m));
            if(m_propertMap.containsKey("text")) mainDiv.appendChild(makePropertyElement("text",m));
            if(m_propertMap.containsKey("uri")) mainDiv.appendChild(makeUriElement(m_propertMap.get("uri"),m));
            if(m_propertMap.containsKey("dateread")) mainDiv.appendChild(makePropertyElement("dateread",m));
        }
        ...
        {
            // assume simple
            if(m_propertMap.containsKey("title"))mainDiv.appendChild(makePropertyElement("title",m));
            if(m_propertMap.containsKey(AUTHORS_IN_LIST)) mainDiv.appendChild(makePropertyElement(AUTHORS_IN_LIST,m));
            if(m_propertMap.containsKey("year")) mainDiv.appendChild(makePropertyElement("year",m));
            if(m_propertMap.containsKey("chaptertitle")) mainDiv.appendChild(makePropertyElement("chaptertitle",m));
            if(m_propertMap.containsKey("journal")) mainDiv.appendChild(makePropertyElement("journal",m));
            if(m_propertMap.containsKey("editor")) mainDiv.appendChild(makePropertyElement("editor",m));
            if(m_propertMap.containsKey("place")) mainDiv.appendChild(makePropertyElement("place",m));
