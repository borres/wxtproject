...
public class RemoteSite
{
    ...
    private Vector<Element> m_selectedModules=null;
            ...
            throws Exception
    {
        ...
        try{m_Doc=domer.makeDomFromUri(m_absLocation);}
        catch(Exception e){throw new Exception("Cannot build DOM");}
        ...
        try{correctModuleAddresses();}
        catch(Exception e1){throw new Exception("Addressing error");}
    ...
    private void collectFromScript(Element defElt)
    {
        ...
        for (int ix = 0; ix < nlist.getLength(); ix++) {
            ...
            if(tmpname.compareTo(Definitions.DEFAULT_ENCODING)==0)
                m_encoding=tmpvalue;
        }
    }
     ...
     */
    public Vector<Element> getSelectedModuleList(PIcommand cmd)
    {
        ...
        // make it, first time
        m_selectedModules=new Vector<Element>(10);
                ...
                for(int mix=0;mix<m_Modules.getLength();mix++)
                {
                    ...
                    if(books.indexOf(","+wanted[wix]+",")!=-1)
                    {
                        ...
                        for(int fix=0;fix<m_selectedModules.size() && !found;fix++)
                            if(m_selectedModules.elementAt(fix)==e)
                                found=true;
                        if(!found)
                            m_selectedModules.add(e);
                    }
     ...
     private String substituteFragments(String loc)
    {
     ...
     public void reAddress(Document doc,String refaddress)
     {
        ...
        for (Iterator<String> k = keys.iterator(); k.hasNext();)
        {
            ...
            // while testing
            //if(tag.compareTo("img")!=0) continue;
