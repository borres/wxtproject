...
public class TXTContent  extends Content
{
    ...
    static protected String prepareTheText(String loadedText,Module mod,PIcommand cmd)
    {
        ...
        if((cmd.paramExist(PIcommand.REPLACE)) || (cmd.paramExist(PIcommand.REPLACE0)))
        {
            ...
            while(more)
            {
                ...
                if(cmd.paramExist(tmp))
                {
                    ...
                    if(ix >1) // allow som slack in PI-param
                        more=false;
                ix++;
            }
        }
        ...
        if(parts.size()>0)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    Vector<Integer>list=accessutils.getIntegerList(select,parts.size());
                    if(list==null)
                    {
        ...
        // should we do colorcoding of code
        if(cmd.paramExist(PIcommand.CODE))
        {
            if(!result.startsWith("\n"))
                result="\n"+result;
            String codetype=cmd.getValue(PIcommand.CODE);
            result=codeFormatter.produceCodedStringFromString(result,codetype,target_encode,mod);
            // can we make a fragment from this ?
            try{
               Document tmpDoc=domer.produceHTMLPageFromContent(result,mod.getEncoding(),mod);
            }
            catch(Exception ex)
            {
                mod.getReporter().pushMessage("failed_to_produce_code",codetype+ ": make simple");
                result=codeFormatter.produceCodedStringFromString(result,"mlcode",target_encode,mod);
            }
        }
     ...
     */
             ...
             throws Exception
     {
        ...
        // encoding
        source_encode=mod.getDefinitions().getOption(Definitions.DEFAULT_ENCODING);
        ...
        // we transform
        if(trans!=null)
            T=domer.makeTransformedStringFromString(T, trans.getabsoluteURI(), transParameters);
