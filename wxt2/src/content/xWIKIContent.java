 ...
 * @author borres
 */
public class xWIKIContent  extends Content
{
    ...
    public xWIKIContent(Module owner,PIcommand cmd)
    throws Exception
    {
       super(owner,cmd);
    }
    ...
    private static boolean makeBackup(Module mod, PIcommand cmd,Document wDoc)
    {
        ...
        for(int imix=0;imix<imList.getLength();imix++)
        {
            ...
            if(pos!=-1)
            {
                ...
                newFileName=accessutils.cleanStringForUseAsFilePath(newFileName);
                // move files to this new filename
                ...
                else
                {
                    // correct adressing
                    try{
                        URI absImage=accessutils.makeUri(absBackupCat.toString()+"/"+newFileName);
