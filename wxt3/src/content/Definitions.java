...
public class Definitions {
    ...
    /** holding used texformulas as we build */
    ItemIndexHolder m_usedFormulas;
    ...
    public Definitions(Scripthandler sh) {
        ...
        m_usedImages=new ItemIndexHolder();
        m_usedFormulas=new ItemIndexHolder();
    ...
    private void collectFromScript() {
        ...
        m_options.put(TIDY_INPUT, YES); // deafult is tidy with std properties
        ...
        for (int ix = 0; ix < nlist.getLength(); ix++) {
            ...
            try {
                ...
                Formula.makeMathMlFormulasFromOdt(m_reporter, sourceUri, getTempCatalog(), this);
            }
            catch (Exception e)
            {
                //System.out.println(e);
        ...
        for (int ix = 0; ix < nlist.getLength(); ix++) {
            ...
            try {
                ...
                location = substituteFragments(location);
                sourceUri = accessutils.makeAbsoluteURI(location, m_anchorCat);
                Formula.addFormulasFromFormulaFile(m_reporter, sourceUri, m_scriptHandler,this);
            }
            catch (Exception e)
            {
                //System.out.println(e);
     ...
     */
    public void registerFormula(Formula tf)
    {
        String id=tf.getId();
    ...
    public ItemIndexHolder getImageHolder()
    {
        return m_usedImages;
    }

    public ItemIndexHolder getFormulaHolder()
    {
    ...
    public HashMap<String,Formula> getFormulas()
    {
        return m_formulas;
    }
    ...
    public void removeFormulasMadeOntheFly()
    {
        ...
        if(m_formulas==null)
            return;
        try{
            Iterator it=m_formulas.keySet().iterator();
            List<String> removables=new ArrayList<String>();
            while(it.hasNext())
            {
                String key=(String)it.next();
