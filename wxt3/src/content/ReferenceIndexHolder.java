...
public class ReferenceIndexHolder 
{
    ...
    public int theIndexOfReference(Reference r,int startix)
    {
        ...
        {
            Reference thisr=m_def.getReference(m_references.get(ix).getId());
     ...
     */
    public List<String> getSortedReferenceIds()
    {
       ...
       List<String>ret=getAllReferenceIds();
       ...
       boolean ok=false;
        while(!ok)
        {
            ...
            {
                Reference r1=m_def.getReference(ret.get(ix));
                Reference r2=m_def.getReference(ret.get(ix+1));
                if(r1.compareTo(r2)>0)
                {
                    // swap them
                    ok=false;
                    String tmp=ret.get(ix);
                    ret.add(ix+2, tmp);
                    ret.remove(ix);
                }
            }
