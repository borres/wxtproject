...
public class XMLContent  extends Content
{
    ...
    public DocumentFragment getContent(Module mod,PIcommand cmd)
    {
        ...
        {
            // should not happen
            wxtError.makeErrorMsgForDebugging(mod.getDoc(), cmd, "Missing XPATH in XMLContent:getContent");
        ...
        try{
            Document doc=domer.makeTransformedDomFromUriSomeHow(m_absoluteLoadUri,
    ...
    static public DocumentFragment getStaticContent(Module mod,PIcommand cmd)
    {
        ...
        try{
            if(trans!=null)
            {
                //doc=domer.makeTransformedDomFromUri(absoluteUri, trans.getabsoluteURI(), transParameters);
                doc=domer.makeTransformedDomFromUriSomeHow(absoluteUri, trans.getabsoluteURI(), transParameters,
                                                           source_encoding,mod.getReporter());
            }
            else
            {
               //doc=domer.makeDomFromUri(absoluteUri);
