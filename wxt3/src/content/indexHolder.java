...
public class indexHolder {
    ...
    private List<ixEntry> m_indexes;

    public indexHolder()
    {
        m_indexes=new ArrayList<ixEntry>();
    }
    ...
    public void insertWord(Module m,PIcommand cmd,String id)
    {
        ...
        if(m_indexes.size()>0)
        {
            ixEntry cmp=m_indexes.get(0);
            int ix=0;
            while((ix<m_indexes.size()) && 
                  (m_indexes.get(ix).m_word.compareToIgnoreCase(ie.m_word) < 0))
                ix++;
            // we do not want to set in same word with same id again
            if((ix<m_indexes.size())&&
               (m_indexes.get(ix).m_word.compareToIgnoreCase(ie.m_word) == 0)&&
               (m_indexes.get(ix).m_id.compareToIgnoreCase(ie.m_id) == 0))
                return;

            if(ix < m_indexes.size())                
                m_indexes.add(ix, ie);//m_indexes.insertElementAt(ie, ix);
            else
                m_indexes.add(ie);                    
        }
        else
     ...
     * @param m The module
     * @param cmd The comand
     * @return A list with all lines in the list, as elements
     */
    public List<Element>makeIxElementList(Module m,PIcommand cmd)
    {
