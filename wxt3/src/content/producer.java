...
import utils.codeBasics;
import utils.codeFormatter;
import utils.codeGoogleFormatter;
import utils.domer;
...
public class producer {
     ...
     */
    private Comment makeFailureComment(Document doc, PIcommand cmd) 
    {
    ...
    private Comment makeEmptyComment(Document doc,PIcommand cmd)
    {
        ...
        if((cmd.paramExist(PIcommand.REMOVEPARENT))&&
            ...
            cmt=doc.createComment(" "+PIcommand.REMOVEPARENT + " empty: "+cmd.toString()+" ");
        else
            cmt=doc.createComment(" empty: "+cmd.toString()+" ");
        return cmt;
    }
     ...
     */
    public Element makeXLink(Module from, Module to, String element_type,
                              ...
                              boolean bSummary) 
    {
        ...
        if(bSummary)
        {
            ...
            Element fragWrapper=from.getDoc().createElement("div");
            ...
            fragWrapper.appendChild(from.getDoc().importNode(theFragmentDF, true));
             
            // add it to the packgae that already contains the link
    ...
    {
        // PI:control: value or id
        //LATEXFormulas formulas=mod.getDefinitions().getFormulas();
        //id,location,source,subtext,color,size,width,height
        DocumentFragment df=mod.getDoc().createDocumentFragment();
        ...
        if(cmd.paramExist(PIcommand.STYLE))style=cmd.getValue(PIcommand.STYLE);       
        if(cmd.paramExist(PIcommand.ID))id=cmd.getValue(PIcommand.ID);
        if(cmd.paramExist(PIcommand.LOCATION))location=cmd.getValue(PIcommand.LOCATION);
        if(cmd.paramExist(PIcommand.SOURCE))source=cmd.getValue(PIcommand.SOURCE);
        if(cmd.paramExist(PIcommand.SUBTEXT))subtext=cmd.getValue(PIcommand.SUBTEXT);
        ...
        if(cmd.paramExist(PIcommand.TYPE))
        {
            ...
            try
            {
                ...
                {
                    // should not happen
                    df.appendChild(wxtError.makeErrorMsgForDebugging(mod.getDoc(), cmd, "unknown formula type"));
                    df.appendChild(makeFailureComment(mod.getDoc(), cmd));
            ...
            // register it
            if(fma!=null)
                    mod.getDefinitions().registerFormula(fma);
       }
        ...
        if(id!=null)
        {
            ...
            if(mod.getDefinitions().getFormula(id)!=null)
                globalindex=mod.getDefinitions().getFormulaHolder().addItemReference(id,mod)+1;
        ...
        {
            // should not happen
            df.appendChild(wxtError.makeErrorMsgForDebugging(mod.getDoc(), cmd, "formula without latex and id in producer:ODTFormula"));
            df.appendChild(makeFailureComment(mod.getDoc(), cmd));
        }

        Formula laf=mod.getDefinitions().getFormula(id);
        if(laf!=null)
        {
            Element fspan=mod.getDoc().createElement("span");           
            fspan.setAttribute("class", cmd.getCommandStyleName());
            fspan.appendChild(laf.getDisplayElement(mod,cmd));
    ...
    public DocumentFragment makePI_Image(Module mod, PIcommand cmd)
    {
        ...
        if(cmd.paramExist(PIcommand.ID))
        {
            ...
            if(mod.getDefinitions().getImage(sid)!=null)
                globalindex=mod.getDefinitions().getImageHolder().addItemReference(sid,mod)+1;
        ...
        Image im=mod.getDefinitions().getImage(imStr);
        if(im==null)
        {
        ...
        // an image ?
        if(cmd.getCommand().compareTo(PIcommand.IMAGE)==0)
        {
            // no display ?
            ...
            // will display
            return im.produceImage(mod, cmd);
        }
        
        // then we must have an IMAGETHUMB
        return im.produceSimpleThumbImage(mod, cmd);
    }
    ...
    public DocumentFragment makePI_FormulaList(Module mod, PIcommand cmd)
    {
        ...
        for(String id : usedIds)
        {
            ...
            Element box=frm.produceFormulaListElement(mod, cmd,mod.getDefinitions().getFormulaHolder().getAllModulesUsing(id));
    ...
    {        
       // PI control:
        ...
        if(cmd.paramExist(PIcommand.SOURCELOCATION))
        {
            ...
            { // should parse the text, just copy now
    ...
    private DocumentFragment makeModuleSortedRefereceList(Module mod, PIcommand cmd)
    {
        ...
        int cols =1;
        if(cmd.paramExist(PIcommand.COLS))
            cols=cmd.getValueAsInteger(PIcommand.COLS);
        Element outerWrapper=mod.getDoc().createElement("div");
        if(cols > 0)
            outerWrapper.setAttribute("class", cmd.getCommandStyleName()+String.valueOf(cols));
        else
        ...
        {
            List<String> refs=m.getReferenceHolder().getSortedReferenceIds();
            // any refs ?
            if(refs.isEmpty())
                continue;
 
            // refereenc from mod to m
            Element e = makeXLink(mod, m, "div", PIcommand.WXTSTYLEPREFIX+PIcommand.XLINK, NO_LEVEL_ADJUST,true);
            ...
            {
                String ids=refs.get(rix);
                Reference r=mod.getDefinitions().getReference(ids);
                listWrapper.appendChild(r.produceListElement(mod,style));
           }
    ...
    public DocumentFragment makePI_ReferenceList(Module mod, PIcommand cmd)
    {
        ...
        if(!is_local)
        {
            ...
            {
                int index=ix+1;
                String ids=refs.get(ix);
                Reference r=mod.getDefinitions().getReference(ids);
                 olElement.appendChild(r.produceListElement(mod, form));
    ...
    public DocumentFragment makePI_Fragment(Module mod, PIcommand cmd)
    {
        ...
        // do we have a fragmentwith the give id
        ...
        if (frag != null) 
        {
            ...
            if(displayType.compareTo(PIcommand.ID)==0)
            {
                ...
                spanElt.appendChild(mod.getDoc().createTextNode(frag.getId()));
            ...
            if(displayType.compareTo(PIcommand.SHORT)==0)
            {
                Element spanElt=mod.getDoc().createElement("span");
                spanElt.setAttribute("class", PIcommand.SKIPTRANSLATE);
                spanElt.appendChild(mod.getDoc().createTextNode(frag.getShort()));
                df.appendChild(spanElt);
        ...
        else 
        {
            ...
            else if (id.compareToIgnoreCase(PIcommand.FID) == 0)
            {
                val = mod.getID();
            } 
            ...
            else if(id.compareToIgnoreCase(PIcommand.AUTHOR) == 0)
            {
               ...
               if(au==null)
               {
                    // should not happen, will produce "at least" siteauthor
                    df.appendChild(makeFailureComment(mod.getDoc(), cmd));
               ...
               if(displayType.compareTo(PIcommand.ID)==0)
               {
                  // id and we dont want translate
                    Element spanElt=mod.getDoc().createElement("span");
                    spanElt.setAttribute("class", PIcommand.SKIPTRANSLATE);
                    spanElt.appendChild(mod.getDoc().createTextNode(au.getId()));
                    df.appendChild(spanElt);
               ...
               if( (displayType.compareTo(PIcommand.SHORT)==0))
               {
                   // short and we dont want translate
                    Element spanElt=mod.getDoc().createElement("span");
                    spanElt.setAttribute("class", PIcommand.SKIPTRANSLATE);
                    spanElt.appendChild(mod.getDoc().createTextNode(au.getShort()));
                    df.appendChild(spanElt);
                  // df.appendChild(mod.getDoc().createTextNode(au.getShort()));
                   return df;
               }
               // full - assume tranlation is given in fragment def
               df.appendChild(mod.getDoc().importNode(au.getFragment(), true));
               return df;
            }
            else 
            {
    ...
    public DocumentFragment makePI_XLink(Module mod, PIcommand cmd) 
    {
        ...
        if (mid.compareTo(PIcommand.NEXT) == 0)
        {
            refMod = null;
            int ix=modix+1;
            while((ix<mList.size())&&(!mList.get(ix).getLinkable())) ix++;
            if(ix < mList.size())
                refMod=mList.get(ix);
        ...
        else if (mid.compareTo(PIcommand.PREV) == 0)
        {
            refMod = null;
            int ix=modix-1;
            while((ix>=0)&&(!mList.get(ix).getLinkable())) ix--;
            if(ix >=0)
                refMod=mList.get(ix);
            if (refMod == null) 
                refMod = mod;
    ...
    public DocumentFragment makePI_AuthorSimpleList(Module mod, PIcommand cmd)
    {
        ...
        // old:
        //Element wrapperOuter=doc.createElement("div");
        //wrapperOuter.setAttribute("class", cmd.getCommandStyleName());
        ...
        {
            wrapperOuter=doc.createElement("span");
            wrapperOuter.setAttribute("class", cmd.getCommandStyleName());
        }
        ...
        for (int ix=0;ix<authors.length;ix++)
        {
            ...
            {
                Node tmp=doc.importNode(au.getFragment(),true);
                // since we get a documentfragment:
                tmp=tmp.getFirstChild(); 
                if((tmp==null) ||(tmp.getNodeType()!=Node.ELEMENT_NODE))
                    auelt.setAttribute("class", PIcommand.SKIPTRANSLATE);
            ...
            {
                auelt.setAttribute("class", "authorname"+" "+PIcommand.SKIPTRANSLATE);
                if(displayType.compareTo(PIcommand.ID)==0)
                    auelt.appendChild(doc.createTextNode(au.getId()));
                else
                    auelt.appendChild(doc.createTextNode(au.getShort()));
                auelt.setAttribute("class",PIcommand.SKIPTRANSLATE);
            }
    ...
    public DocumentFragment makePI_AuthorOfModules(Module mod, PIcommand cmd)
    {
        ...
        for (int ix=0;ix<authors.length;ix++)
        {
            ...
            if(shortform)
            {
                listhead.appendChild(doc.createTextNode(au.getShort()));
            ...
            innerwrapper.appendChild(listhead);
            ...
            for(Module m : resultList)
            {
                if(m.getAuthor().getId().compareTo(au.getId())==0)
                {
                    ...
                    Element listelt=makeXLink(mod, m, "dd", null, NO_LEVEL_ADJUST, true);
                    innerwrapper.appendChild(listelt);
                }
            }
    ...
    public DocumentFragment makePI_IXWord(Module mod, PIcommand cmd)
    {
        ...
        if(markWord.compareToIgnoreCase(PIcommand.NONE)!=0)
        {
            ...
            //Element nameElt=mod.getDoc().createElement("div");
            //nameElt.setAttribute("id", wid);
        ...
        for(int ix=0;ix < imList.getLength();ix++)//drop dummy
        {
            ...
            try{
                Image theImage=new Image(docCat+"/"+relfilepath,
                             ...
                             tmpid);
                mod.getDefinitions().registerNewImage(theImage);
                mod.getDefinitions().getImageHolder().addItemReference(tmpid,mod);
            }
            catch(Exception rex)
            {
