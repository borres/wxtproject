...
public final class PIcommand extends java.lang.Object 
{
   ...
   // used to make classnames matching PI-name
    ...
    {
        // all
        if(m_Cmd.compareToIgnoreCase(REMOVEPARENT)==0)
        {
             if( (m_params.get(REMOVEPARENT).compareTo(YES)!=0) &&(m_params.get(REMOVEPARENT).compareTo(NO)!=0))
       ...
       if(m_Cmd.compareToIgnoreCase(XLINK)==0)
       {
        ...
        if((m_Cmd.compareToIgnoreCase(FORMULA)==0))
        {
           ...
           if( m_params.containsKey(TYPE))
           {
               ...
               {
                   return TYPE+" must be "+LATEX+" , "+GOOGLE+" , "+IMAGE+" o "+MATHML;
               }
           }
