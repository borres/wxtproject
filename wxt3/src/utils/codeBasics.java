/*
 * Basic values describing available code types
 */
...
/**
 * Handling colorcoding basic for bot Google and WXT codeformatter
 * @author bs
 */
public class codeBasics {
    ...
    {
        if(c.compareToIgnoreCase(XMLCODE)==0) return LANG_XML;
        if(c.compareToIgnoreCase(MLCODE)==0) return LANG;
        if(c.compareToIgnoreCase(JAVACODE)==0) return LANG_J;
        if(c.compareToIgnoreCase(JAVASCRIPTCODE)==0) return LANG_JS;
        if(c.compareToIgnoreCase(CCODE)==0) return LANG_C;
        if(c.compareToIgnoreCase(CPPCODE)==0) return LANG_CPP;
        if(c.compareToIgnoreCase(CSHARPCODE)==0) return LANG_CSH;
        if(c.compareToIgnoreCase(PYTHONCODE)==0) return LANG_PY;
        if(c.compareToIgnoreCase(XSLTCODE)==0) return LANG_XML;
        if(c.compareToIgnoreCase(ASPXCODE)==0) return LANG_ASPX;
        if(c.compareToIgnoreCase(CS3CODE)==0) return LANG_CS3;
        if(c.compareToIgnoreCase(MATLABCODE)==0) return LANG_MAL;
        if(c.compareToIgnoreCase(HTMLCODE)==0) return LANG_HTML;
        if(c.compareToIgnoreCase(CSSCODE)==0) return LANG_CSS;
        return c;
    }
