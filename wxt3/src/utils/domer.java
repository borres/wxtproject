...
public class domer extends java.lang.Object {
    ...
    static public DocumentBuilder makeDocBuilder(boolean namespaceAware)
        ...
        dFact.setAttribute("http://apache.org/xml/features/dom/defer-node-expansion",new Boolean(false));
   ...
   /** 
    ...
    public static Document makeDomFromStringSomeHow(String S,String encoding,reporter theReporter)
        ...
        {
            if(m_Def.getTidyEngine()!=null)
            {    
                try
                {
                    // --------- attempt to tidy, final attempt ----------
                    //S=accessutils.tidyAndMakeXML(S,encoding,theReporter);
                    S=m_Def.getTidyEngine().tidyAndMakeXML(S,encoding,theReporter);
                    return makeDomFromString(S);
                }
    ...
    throws Exception{        
        try{                       
            ...
            // should make sure file is closed if it is a file
            // so we dont get sharing troubble in editor
    ...
    throws Exception
    {
        ...
        catch(Exception ex)
        {
            if(m_Def.getTidyEngine()!=null)
            {
                // --------- attempt to tidy, final attempt ----------
                try{
                    //theString=accessutils.tidyAndMakeXML(theString,encoding,theReporter);
                    theString=m_Def.getTidyEngine().tidyAndMakeXML(theString,encoding,theReporter);
                    return makeTransformedDomFromString(theString, transUri, parameters);
                }
                catch(Exception sex3)
                {
    ...
    throws Exception{
      ...
      * @throws Exception when save is not accomplished
      */
    public static String OLDsaveDomToString(Document doc,String encoding,boolean bIndent,String output)
    throws Exception{
        if(doc==null)
            throw(new Exception("No_dom_to_save"));
        // find out how we should form output
        ...
        try{            
            ...
            boolean is_HTML5=false;
            /*
             if((name!=null)&&(name.compareToIgnoreCase("html")==0)
                    &&(pid==null)&&(sid==null))
            {
                //is_HTML5=true;
            }
     ...
     * @param mod The module where we steal the header element
     * @return A Document if we succed, otherwise null
     */
     public static Document produceHTMLPageFromContent(String content,String encoding,Module mod)
     {
         Document motherDoc=mod.getDoc();
         if(encoding==null)
             encoding=default_encoding;
         ...
         DocumentType dt=motherDoc.getDoctype();
         if(dt!=null)
             XMLHeader+="\n"+"<!DOCTYPE html PUBLIC \""+dt.getPublicId()+"\" \""+dt.getSystemId()+"\">";
         ...
         if((motherDoc.getElementsByTagName("head").getLength()>0) &&
            (Doc.getElementsByTagName("head").getLength()>0))
         {
             try{
                 Doc.setDocumentURI(motherDoc.getDocumentURI());
