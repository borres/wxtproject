...
public class GoogleTexFormula extends Formula {
    ...
    public GoogleTexFormula(Module mod, PIcommand cmd)
        ...
        super(mod, cmd);
        m_madeOnTheFly = true;
        m_formulaType = PIcommand.GOOGLE;
        m_backupCatalog = 
                mod.getDefinitions().getBuildCatalog() + "/" + Definitions.GOOGLEBACKUP;
        if (cmd.paramExist(PIcommand.SOURCE)) {
            m_formulaSource = cmd.getValue(PIcommand.SOURCE);
            convertToLocal(cmd);
            leaveMessage();
        } else {
            // location
    ...
    /**
     * Constructing a formula from an alement in a formula file
     * @param def The active Definitions
     * @param elt The element describing the fomula
    ...
    public GoogleTexFormula(Definitions def, Element elt)
        ...
        m_formulaSource = ((Element) (elt.getElementsByTagName(PIcommand.VALUE).item(0))).getTextContent().trim();
    ...
    public String getFormulaLocation(Module mod) {
        // we try to use a locally stored copy
        try {
    ...
    /**
     * Makes a local copy of google formula
     * if google lets us down, or we operate without net
     */
    private String convertToLocal(PIcommand cmd) {
        String pathmodifier = "";
        String tmp_formula_source = GOOGLECHARTURI;
        String tmp_formula = m_formulaSource;
        if (cmd != null) {
            String color = null;
        ...
        // pick up the source
        String googleSource = GOOGLECHARTURI + GOOGLELATEX + cleanAndEncodeFormula(m_formulaSource);
        googleSource = googleSource.replace(' ', '_');
    ...
    /**
     * Leave a txt-file in the backup catalog
     */
