...
public class Module extends DefaultMutableTreeNode {
    ...
    private Module(Scripthandler scriptH) {
        ...
        m_ScriptContent = new ArrayList<>(1);
        m_PIContent = new HashMap<>(2);
        ...
        m_sourceUris = new ArrayList<>(10);
        m_referenceHolder = new Index();
        m_foundRefLists = new ArrayList<>();
    ...
    public Module(Module father, Element elt, Scripthandler scriptH)
        ...
        //   importxml, importtxt, importhtml, importdb
        // respectively
        ...
        if (m_scriptElement.hasChildNodes()) {
            ...
            while (nod != null) {
                if (nod.getNodeType() == Node.ELEMENT_NODE) {
                    ...
                    } else if (celt.getNodeName().compareToIgnoreCase(HTMLCONTENT) == 0) {
                        try {
                            m_ScriptContent.add(new ScriptContent(celt, this));
                        } catch (Exception e) {
                            m_Reporter.pushMessage("cannot_establish_htmlcontent");
                        }
    ...
    private void pickUpOutputFormat() {
        // pickup outputmethod
        m_outputFormat =m_scriptHandler.getOption(Options.OUTPUT_FORMAT);
        if (m_scriptElement.hasAttribute(Options.OUTPUT_FORMAT)) {
            String tmp = m_scriptElement.getAttribute(Options.OUTPUT_FORMAT).trim();
            tmp = tmp.toLowerCase();
    ...
    public void build() {
        ...
        // Get all importxml, importhtml, importtxt, PIs without location or location=content
        ...
        for (Node pi_node : pilist) {
            ...
            try {
                ...
                String cmd_cmd = cmd.getCommand().toLowerCase();
                //---------------------------------------------
                // picking up import as stated in the script (without location)
                switch(cmd_cmd){
                    
                    case PIcommand.IMPORTXML:
                        ...
                        if ((!cmd.paramExist(PIcommand.LOCATION))
                                || (cmd.getValue(PIcommand.LOCATION).equals(PIcommand.CONTENT))) {
                    ...
                    case PIcommand.IMPORTHTML:
                        if ((!cmd.paramExist(PIcommand.LOCATION))
                                || (cmd.getValue(PIcommand.LOCATION).equals(PIcommand.CONTENT))) {
                            DocumentFragment df = m_Producer.import_ContentToTemplate(this, cmd, HTMLCONTENT);
                            insertDF(df, pi_node);
                            //pi_node.getParentNode().replaceChild(df, pi_node);
                        }
                        break;
                        
                    case PIcommand.IMPORTDB:
                        ...
                        break;
                    // there are normall others, but we dont care abourt them now
                    // se below
               }//switch
            }// try
            catch (Exception e) {
        ...
        while (doItAgain && (count < MAXDEPTH)) {
            ...
            for (Node pi_node : pilist) {
                ...
                DocumentFragment df;
                //-------------------------------------
                // we are ready for the loooong switch, picking up all PIs
                switch(cmd_cmd){
                    
                //-------------------------------------
                // XML content: <?_wxt importxml location xpath
                    case PIcommand.IMPORTXML:
                        df = m_Producer.makePI_XMLContent(this, cmd);
                        insertDF(df, pi_node);
                        doItAgain = true;
                        break;
                
                //-------------------------------------
                // HTML content: <?_wxt importhtml location ...
                // or <?_wxt importremotehtml location ..
                    //case PIcommand.IMPORTREMOTEHTML:
                    case PIcommand.IMPORTHTML:
                        df = m_Producer.makePI_HTMLContent(this, cmd);
                        insertDF(df, pi_node);
                        doItAgain = true;
                        break;
                ...
                // TXT content: <?_wxt importtxt location ....  
                    case PIcommand.IMPORTTXT:
                        df = m_Producer.makePI_TXTContent(this, cmd);
                        insertDF(df, pi_node);
                        doItAgain = true;
                        break;
                //---------------------------------------
                // DB content: <?_wxt importdb  ....
                    case PIcommand.IMPORTDB:
                        df = m_Producer.makePI_DBContent(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //---------------------------------------
                // fragment: <?_wxt fragment id ?>
                    case PIcommand.FRAGMENT:
                        df = m_Producer.makePI_Fragment(this, cmd);
                        insertDF(df, pi_node);
                        doItAgain = true;
                        break;
                //------------------------------------
                // formula: <?_wxt formula  ....
                    case PIcommand.FORMULA:
                        df = m_Producer.makePI_Formula(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //---------------------------------------
                // popfragment: <?_wxt popfragment id text?>
                    case PIcommand.POPFRAGMENT:
                        // we send pi_node since we want to place a
                        // popping fragment relative to this
                        df = m_Producer.makePI_PopFragment(this, cmd, pi_node);
                        insertDF(df, pi_node); 
                        break;
                //---------------------------------------
                // xlinx: <?_wxt xlink id 
                    case PIcommand.XLINK:
                         df = m_Producer.makePI_XLink(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //---------------------------------------
                // modulemap: <?_wxt map  
                    case PIcommand.MODULEMAP:
                        df = m_Producer.makePI_ModuleMap(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //---------------------------------------
                // path: <?_wxt path pathdivider
                    case PIcommand.PATH:
                        df = m_Producer.makePI_Path(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //---------------------------------------
                // moduletoc: <?_wxt moduletoc
                    case PIcommand.MODULETOC:
                        if (m_ModTocs == null) {
                ...
                // indexword: <?_wxt ixword word category
                    case PIcommand.IXWORD:
                        df = m_Producer.makePI_IXWord(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //----------------------------------------
                // <_wxt reftest
                    case PIcommand.REFERENCETEST:
                        df = m_Producer.makePI_ReferenceTest(this, cmd);
                        insertDF(df, pi_node);
                        break;
                //--------------------------------------
                // indexlist: <?_wxt ixtable category root
                    case PIcommand.IXTABLE:
                ...
                //---------------------------------------
                // <?_wxt imagethumb
                        case PIcommand.IMAGETHUMB:
                            df = m_Producer.makePI_Image(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //--------------------------------------
                // <?_wxt image
                        case PIcommand.IMAGE:
                            df = m_Producer.makePI_Image(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //--------------------------------------
                // <?_wxt imagelist
                        case PIcommand.IMAGELIST:
                ...
                // <?_wxt ref  
                        case PIcommand.REF:
                            if (cmd.paramExist(PIcommand.ID)) {
                                ...
                                Reference theRef = m_Definitions.getReference(sid);
                                ...
                                // order we want
                                if (m_scriptHandler.getOption(Options.REFERENCE_INDEXING).equalsIgnoreCase(Options.GLOBAL)) {
                                ...
                                df = m_Producer.makePI_Reference(this, cmd);
                                insertDF(df, pi_node);
                            } 
                            else{
                ...
                // <?_wxt date 
                        case PIcommand.DATE:
                            df = m_Producer.makePI_DateStamp(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt time 
                ...
                //----------------------------------
                // <?_wxt stamp 
                        case PIcommand.STAMP:
                            df = m_Producer.makePI_Stamp(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt modulemenu
                        case PIcommand.MODULEMENU:
                            df = m_Producer.makePI_ModuleMenu(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt authorlist
                        case PIcommand.AUTHORLIST:
                            df = m_Producer.makePI_AuthorOfModules(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt authors
                        case PIcommand.AUTHORS:
                            df = m_Producer.makePI_AuthorSimpleList(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //------------------------------------
                // <?_wxt gadget location width height title
                        case PIcommand.GADGET:
                            df = m_Producer.makePI_Gadget(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt popup 
                        case PIcommand.POPUP:
                            df = m_Producer.makePI_Popup(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt expand 
                        case PIcommand.EXPAND:
                            m_Reporter.pushMessage("PI_is_removed", PIcommand.EXPAND, PIcommand.EXPANDABLE);
                            df = m_Producer.makePI_ExpandAJAX(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt expandsimple
                        case PIcommand.EXPANDSIMPLE:
                            m_Reporter.pushMessage("PI_is_deprecated", PIcommand.EXPANDSIMPLE, PIcommand.EXPANDABLE);
                            df = m_Producer.makePI_Expandable(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt expandable
                        case PIcommand.EXPANDABLE:
                            df = m_Producer.makePI_Expandable(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //----------------------------------
                // <?_wxt demolink 
                        case PIcommand.DEMOLINK:
                            df = m_Producer.makePI_DemoLink(this, cmd);
                            insertDF(df, pi_node);
                            break;
                //------------------------------------
                // <?_wxt footnote
                ...
                // we expand it below
                        case PIcommand.FOOTNOTE:
                            m_FootNoteNode = pi_node;
                            break;                            
                        
                //------------------------------------
                // we dont know any other PIs
                        default:
        ...
        if (m_scriptHandler.getOption(Options.PREFORMAT_LANGUAGE).compareTo(Options.NO) == 0) {
...
/**
    ...
    public void buildSecondPhase() {
        ...
        for (Node pi_node : pilist) {
            ...
            // <?_wxt collect
                case PIcommand.COLLECT:
                    df = m_Producer.makePI_Collect(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //--------------------------------------
            // collect remote: <?_wxt collect-remote
                case PIcommand.COLLECTREMOTE:
                    df = m_Producer.makePI_CollectRemote(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //--------------------------------------
            // collectsummary: <?_wxt collectsummary
                case PIcommand.COLLECTSUMMARY:
                    df = m_Producer.makePI_CollectSummaries(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //--------------------------------------
        ...
        for (Node pi_node : pilist) {
            ...
            // indexlist: <?_wxt ixtable category root
                case PIcommand.IXTABLE:
                    df = m_Producer.makePI_IXTable(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //--------------------------------------
            // imagelist
                case PIcommand.IMAGELIST:
                    df = m_Producer.makePI_ImageList(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //--------------------------------------
            // formulalist
                case PIcommand.FORMULALIST:
                    df = m_Producer.makePI_FormulaList(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //-------------------------------------
            // moduletocfinal:<?_wxt moduletocfinal
                case PIcommand.MODULETOCFINAL:
                    df = m_Producer.makePI_ModuleTocFinal(this, cmd);
                    insertDF(df, pi_node);
                    break;
            //------------------------------------
            // we dont know others that should be left here
                default:
                    m_Reporter.pushMessage("unknown_pi", cmd.toString());
    ...
    public boolean save() {
        if (m_transformation != null) {
            ...
            try {
                String b = m_scriptHandler.getOption(Options.INDENT_OUTPUT);
    ...
    public String getOutputFormat() {
        if (m_outputFormat == null) {
            return m_scriptHandler.getOption(Options.OUTPUT_FORMAT);
        }
    ...
    private void insertDF(DocumentFragment df, Node piNode) {
        if ((df.hasChildNodes()) && (df.getFirstChild().getNodeType() == Node.COMMENT_NODE)) {
            ...
            if (R.indexOf(PIcommand.REMOVEGRANDPARENT) != -1) {
                piNode.getParentNode().getParentNode().getParentNode().replaceChild(df, piNode.getParentNode().getParentNode());
                return;
            }
        }
    ...
    public String getEncoding() {
        ...
        return m_scriptHandler.getOption(Options.DEFAULT_ENCODING);
    }
    ...
    public String Inspector() {
        ...
        for (int ix = 0; ix < m_ScriptContent.size(); ix++) {
            ...
            sb.append("<tr>");
            sb.append("<td>XMLContent").append(bup).append("</td>");
            sb.append(tdstartcol2).append(m_ScriptContent.get(ix).getAbsoluteEffectiveUri().toString()).append("</td>");
            sb.append("</tr>");
        }
        for (int ix = 0; ix < m_ScriptContent.size(); ix++) {
            ...
            sb.append("<tr>");
            sb.append("<td>TXTContent").append(bup).append("</td>");
            sb.append(tdstartcol2).append(m_ScriptContent.get(ix).getAbsoluteEffectiveUri().toString()).append("</td>");
            sb.append("</tr>");
        }
        for (int ix = 0; ix < m_ScriptContent.size(); ix++) {
            if (!(m_ScriptContent.get(ix).getType().compareTo(Module.DBCONTENT) == 0)) {
                continue;
            }
            String connect =m_ScriptContent.get(ix).getConnectionString();
            if(connect==null) {
                connect="-";
            }

            sb.append("<tr>");
            sb.append("<td>DBContent</td>");
            sb.append(tdstartcol2).append(connect).append("</td>");
            sb.append("</tr>");
        }
        sb.append("</table>");
