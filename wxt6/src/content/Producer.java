...
public class Producer {
     ...
     * Columns is applied by CSS in the using instance. This 
    ...
    /**
     * Produce a documentfragment with content from a non-scriptet html import
     * 
     * @param mod The Module requesting the content
     * @param cmd The PIcommand describing the request
     * @return a DocumentFragment, always
     */
    public DocumentFragment makePI_HTMLContent(Module mod, PIcommand cmd) {
        ...
        if (!cmd.paramExist(PIcommand.LOCATION)) {
            // should not happen
            mod.getReporter().pushMessage("bad_processing_instruction", "Missing " + PIcommand.LOCATION);
            df.appendChild(wxtError.makeErrorMsgForDebugging(mod, cmd, "missing location in Producer:HTMLContent"));
        ...
        String s=cmd.getValue(PIcommand.LOCATION);
        if(s==null){
            df = mod.getDoc().createDocumentFragment();
            df.appendChild(makeFailureComment(mod, cmd));
            mod.getReporter().pushMessage("missing_location_inhtmlcontent");
            return df;           
        }
        
        // we should generalize this to foreign webpages
        if(cmd.paramExist(PIcommand.REMOTE) && 
                (cmd.getValue(PIcommand.REMOTE).equals(PIcommand.YES)))
        {
            try {
                cont = new RemoteHTMLContent(mod, cmd);
                df = cont.getContent(mod, cmd);
            } catch (Exception ex) {
                df = null;
            }
        }
        ...
        //---------- ?
        try {
            abs_uri = cont.getActualSourceLocation();
            //DocumentFragment fragment, Module target, URI sourceURI
            correctAddressing(df, mod, abs_uri);
        } catch (Exception ex) {
            mod.getReporter().pushMessage("bad_processing_instruction", cmd.toString(), ex.getMessage());
            df.appendChild(makeFailureComment(mod, cmd));
    ...
    public DocumentFragment makePI_Popup(Module mod, PIcommand cmd) {
        ...
        if (cmd.paramExist(PIcommand.SOURCELOCATION)) {
            ...
            if (mod.getScriptHandler().getOption(Options.PREFORMAT_LANGUAGE).compareTo(Options.NO) == 0) {
    ...
    public DocumentFragment makePI_Expandable(Module mod, PIcommand cmd) {
        ...
        if (cmd.paramExist(PIcommand.EXPANDED)) {
            ...
            expandit = expandvalue.equalsIgnoreCase(PIcommand.YES);
        }
        // general expand option (expand-all) should override the attribute if set to yes
        if (!expandit) {
            String tmp = mod.getScriptHandler().getOption(Options.EXPAND_ALL);
            if ((tmp != null) && (tmp.equalsIgnoreCase(PIcommand.YES))) {
                expandit = true;
            }
        }
        //--------------
    ...
    private DocumentFragment makeCompactSortedReferenceList(Module mod, PIcommand cmd) {
        ...
        innerWrapper.setAttribute("class", cmd.getCommandStyleName() + "inner" + " " + mod.getScriptHandler().getOption(Options.REFERENCE_FORM));
        outerWrapper.appendChild(innerWrapper);
        List<Indexable> usedRefs = new ArrayList<>();
    ...
    public DocumentFragment makePI_ReferenceList(Module mod, PIcommand cmd) {
        ...
        if ((mod.getScriptHandler().getOption(Options.REFERENCE_INDEXING).compareTo(Options.LOCAL) == 0)
                && (!is_local)) {
        ...
        String refType = mod.getScriptHandler().getOption(Options.REFERENCE_FORM);
        ...
        if (!is_local) {
            ...
            for (int ix = 0; ix < refs.size(); ix++) {
                ...
                try{
                    ...
                    String ids = refs.get(ix).getId();
                    Reference r = mod.getDefinitions().getReference(ids);
                    if (mod.getScriptHandler().getOption(Options.REFERENCE_INDEXING).compareTo(Options.GLOBAL) == 0) {
                    ...
                    // r==null ?
                    mod.getReporter().pushMessage("no_reference", mod.getName(), cmd.toString());
                }
            }
        }
    ...
    public DocumentFragment makePI_CollectRemote(Module mod, PIcommand cmd) {
        ...
        for (int wix = 0; wix < actualModules.size(); wix++) {
            ...
            try {
                remoteDoc = domer.makeDomFromUri(theURI,false, theEncoding);
    ...
    public DocumentFragment makePI_Stamp(Module mod, PIcommand cmd) {
        ...
        spanElt.setAttribute("class", cmd.getCommandStyleName());
        Element aelt = mod.getDoc().createElement("a");
        aelt.setAttribute("href", "https://borres.hiof.no/wxtsite/");
        aelt.setAttribute("class", "external");
        String version = resourceHandler.getVersion();
    ...
    public DocumentFragment import_ContentToTemplate(Module mod, PIcommand cmd, String contentType) {
        ...
        for (ScriptContent cont : list) {
            ...
            if ((cmdId == null) || (cmdId.compareTo(cont.getId()) == 0)) {
                ...
                if (contentType.equals(Module.TXTCONTENT)) {
                    tmpdf = makePI_TXTContent(mod, cmd);
                } else if (contentType.equals(Module.XMLCONTENT)) {
                    tmpdf = makePI_XMLContent(mod, cmd);
                } else if (contentType.equals(Module.HTMLCONTENT)) {
                    tmpdf = makePI_HTMLContent(mod, cmd);
                }else if (contentType.equals(Module.DBCONTENT)) {
