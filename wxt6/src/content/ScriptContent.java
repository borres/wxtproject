 ...
 * XMLContent,TextContent,WIKIContent or DBContent
 * This content is presented as contnet-elem,ents in the script
 * and are included in the template without a location-parameter
 * @author borres
 */
public class ScriptContent {
    ...
    public ScriptContent(Element celt, Module owner)
        ...
        if (       (!m_type.equals(Module.TXTCONTENT))
                && (!m_type.equals(Module.XMLCONTENT))
                && (!m_type.equals(Module.HTMLCONTENT))
                && (!m_type.equals(Module.DBCONTENT))) {
            // should not happen
    ...
    public boolean usingBackup() {
        if(   (m_absoluteBackupUri!=null)
            &&(m_absoluteBackupUri.toString().equals(m_absoluteEffectiveUri.toString()))) {
            return true;
        }
        return false;
    }
