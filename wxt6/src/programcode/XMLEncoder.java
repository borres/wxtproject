...
public class XMLEncoder extends DefaultHandler2 {
    ...
    /**
     * constructor
     * @param makehtml If we want HTML
     */
     ...
     * @param theReporter The active reporter
     * @return The prepared string,or null if we encounter problems
     */
    public String parseText(String source, String enc, reporter theReporter,boolean dotidy) {
        ...
        try {
            ...
            System.out.println(se.getMessage() + " in: " + source);
            //theReporter.pushSimpleMessage(se.getMessage());
            theReporter.pushMessage("error_xml_encoder", "Not well formed XML", se.getMessage());
            
            // try to tidy
            if(!dotidy) {
                return null;
            }
            if(CodeFormatter.s_Def==null) {
                return null;
            }
            theReporter.pushMessage("attempt_to_tidy");
            ITidyXML td=CodeFormatter.s_Def.getTidyTool();
            try{
                String res=td.tidyAndMakeXML(source, enc, theReporter);
                res= parseText(res, enc, theReporter,false);
                return res;
            }
            catch(Exception ex){
                return null;
            ...
            System.out.println(pce.getMessage() + " in: " + source);
            //theReporter.pushSimpleMessage(pce.getMessage());
            theReporter.pushMessage("error_xml_encoder", "parser config", pce.getMessage());
            return null;
        } catch (Exception nex) {
            System.out.println(nex.getMessage() + " in: " + source);
            //theReporter.pushSimpleMessage(nex.getMessage());
            theReporter.pushMessage("error_xml_encoder", "", nex.getMessage());
            return null;
        }
    ...
    public void startElement(String uri, String localName, String qName, Attributes attributes)
        ...
        bResult.append("&lt;<span class=\"" + TAGNAME + "\">").append(qName).append("</span>").append(eltString).append("&gt;");//result += "&lt;<span class=\""+TAGNAME+"\">"+qName+"</span>"+eltString+"&gt;";
        lastDone = START;
    ...
    public void processingInstruction(String target, String data) {
        ...
        bResult.append("&lt;?<span class=\"" + ATTNAME + "\">").append(target).append("</span> <span class=\"" + ATTVALUE + "\">").append(data).append("</span>?&gt;");//result+="&lt;?<span class=\""+ATTNAME+"\">"+target+"</span> <span class=\""+ATTVALUE+"\">"+data+"</span>?&gt;";
        lastDone = PI;
    }
    ...
    public void comment(char[] ch, int start, int length)
        ...
        bResult.append("&lt;!--<span class=\"" + COM + "\">").append(t).append("</span>--&gt;");//result+="&lt;!--<span class=\""+COM+"\">"+t+"</span>--&gt;";
        lastDone = COMMENT;
    }
    ...
    public void endElement(String uri, String localName, String qName)
        ...
        if (lastDone == START) {
            ...
            bResult.append("&lt;/<span class=\"" + TAGNAME + "\">").append(qName).append("</span>&gt;");//result += "&lt;/<span class=\""+TAGNAME+"\">"+qName+"</span>&gt;";
        }
        lastDone = END;
        elementDepth -= 1;
    ...
    public void startDocument() {
        bResult.append(xmlheader).append(EOL);//result=xmlheader+EOL;
    }
    ...
    public void startDTD(String name, String publicId, String systemId) {
        ...
        bResult.append("\n&lt;!DOCTYPE ").append(name);//result+="\n&lt;!DOCTYPE " +name;
        if (publicId != null) {
            bResult.append(" PUBLIC " + "\"").append(publicId).append("\"");//result=result+" PUBLIC "+"\""+publicId+"\"";
            if (systemId != null) {
                bResult.append(EOL + "            " + "\"").append(publicId).append("\"");//result=result+EOL+"            "+"\""+publicId+"\"";
            }
        } else if (systemId != null) {
            bResult.append(" SYSTEM " + "\"").append(systemId).append("\"");//result=result+" SYSTEM "+"\""+systemId+"\"";
        }
