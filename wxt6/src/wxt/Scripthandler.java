 ...
 * <ul>
 *  <li>
 *  construct Scripthandler(HashMap<String,String>arguments)
 ...
 * </li><li>
 *  String getVersion()
 *    returns the version of WXT
 * </li><li>
 *  String getLatestVersion()
 *    returns the latest published version of WXT
 * </li><li>
 *  String getOptionDescription()
 *      returns description of options as html(for Panel display in GUI)
 * </li>
 * </ul>
 *
 ...
 * <li>default values, see class Options</li>
 * </ol>
 */
public class Scripthandler {
    ...
    /** the directory path for the script*/
    ...
    /** all definitions, in the one and only Definitions object */
    private Definitions m_Def;
    /** the reporter */
    private reporter m_Reporter;
    /** the Producer */
    private Producer m_Producer;
    // wil need to Options: one based on script and one based on arguments
    /** Optons as read from script*/
    Options m_ScriptOptions;
    /** options corrected from arguments to build or update*/
    Options m_BuildOptions;
    /** when we traverse next and prev, result of getAllModules(null)*/
    List<Module> m_linearModuleList;
    /** flagging that we will build all modules*/
    ...
    public Scripthandler(HashMap<String, String> arguments)
       ...
       // the arguments are: script, modules and all legal build options
        // the script-argument is used to access, construct and analyze the script
        // the modules-argument is not used by the constructor
        
        // we will not need buildoptions here in the construction phase
        m_BuildOptions=null;
        
        // set up an options object to receive options from Definitions
        m_ScriptOptions=new Options(m_Reporter);      
        ...
        resourceHandler.theReporter=m_Reporter;

        // report which WXT engine
        ...
        m_scriptPath=arguments.get(Options.SCRIPT);
        //m_scriptPath=arguments.get("script");
        m_scriptPath = m_scriptPath.replace(" ", "%20");
        
        System.out.println("ScriptHandler: "+m_scriptPath);
        
        m_scriptCatalog = new File(m_scriptPath).getParent();
        
        // set up a definition object withot starting collection
        // m_scriptCatalog must be set when we do this
        m_Def = new Definitions(this);
        // and tell domer about it
        domer.setDefinitions(m_Def);

        //debugging:System.out.println("Scripthandler: starting constructor");


        try {
            //m_scriptCatalog = new File(m_scriptPath).getParent();
            m_scriptPath = m_scriptPath.replace('\\', '/');
            //debugging:System.out.println("Scripthandler: script is "+m_scriptPath);
            ...
            // try to make the scriptdom
            // no stringsource or tidy option here
            m_scriptDoc = domer.makeDomFromUri(m_absoluteScriptUri,true,null);
            
            
            // is it a wxt-script, complete validation below
            if (m_scriptDoc.getDocumentElement().getNodeName().compareTo("wscript") != 0) {
                throw new Exception("not legal wxtcript: no wscript-tag");
            }
            //debugging:System.out.println("Scripthandler has built dom");            
        ...
        // validate the script
        // we will get away without a definitions element
        try {
            // find the schema
            String schemaString = resourceHandler.getScriptSchema();
        ...
        // take care of all definitions, in a Definitions ocject
        NodeList deflist = m_scriptDoc.getElementsByTagName("definitions");
        if (deflist.getLength() == 0) {
            // we add an empty difinitions element
            Element elt=m_scriptDoc.createElement("definitions");
            m_scriptDoc.getDocumentElement().appendChild(elt);
            m_Def.collectFromScript(elt);
        }
        else {
            if (deflist.getLength() > 1) {
                m_Reporter.pushSimpleMessage("Only one definitions element allowed, using first");
            }
            m_Def.collectFromScript((Element) deflist.item(0));
        }


        
        // scriptoptions is now ready since we have got updates from the Definitions-object
        ...
        try {
            ...
            // based on what we find in the script
            // the modules-enrty in fragment (copied to options) is not used
            ...
            // possibly remove this WXT-possibility
    ...
    /**
     * Called from definitions to update options set in script
     */
    public boolean setScriptOption(String key,String value){
        if(m_ScriptOptions.isLegalOption(key)){
            m_ScriptOptions.setOption(key, value);
            return true;
        }
        else 
            return false;
    ...
    public Options getScriptOptions(){
        return m_ScriptOptions;
    }
    
    public String getScriptOption(String key){
          return m_ScriptOptions.getOption(key);
    }
    
   /**
    * We have a few situations where options ar accessed both in establish and build
    * We select the best updated option pack
    */
    public String getOption(String key) {
       if(m_BuildOptions==null) {
            return getScriptOption(key);
        }
        return m_BuildOptions.getOption(key);
    }
    ...
    //--------------  statics to access versionstuff from the GUI --
     /**
     * Access the version as a string: mv.yyyy.mm.dd
     * @return The version compiled
     */
    public static  String getVersion() {
        return resourceHandler.getVersion();
    }

    /**
     * Access the latest version distributed as a string: mv.yyyy.mm.dd
     * @return The version compiled
     */
    public static  String getLatestVersion() {
        return resourceHandler.getLatestDistVersion();
    }
    //---------------------------------------------------------
    ...
    public void buildModules(HashMap<String, String> arguments) {
        ...
        // we set up an option object that copies all options set in construction
        // and we override with arguments
        m_BuildOptions=new Options(m_ScriptOptions);
        String[] modulelist = null;
        if (arguments != null) {
            m_BuildOptions.setAllOptions(arguments);
            // while debugging:
            //System.out.println(m_BuildOptions);
  
            // module list
            String mv=m_BuildOptions.getOption(Options.MODULES);
            if ((mv!=null) && (!mv.equalsIgnoreCase(Options.NO_MODULES))){
                System.out.println("buildModules: " + mv);
                modulelist = mv.split(",");
            }

            // GUI signal: one module with name ALL_MODULES means the whole script
            if ((modulelist != null) && (modulelist.length == 1) && (modulelist[0].equalsIgnoreCase(Options.ALL_MODULES))) {
        ...
        if (modulelist == null) {
            ...
            m_Reporter.pushSimpleMessage("Start building selected module(s):" +
                    m_BuildOptions.getOption(Options.MODULES));
        }
        ...
        for (Module mod : m_RootModules) {
            ...
            Enumeration<Module> all = mod.preorderEnumeration();
            while (all.hasMoreElements()) {
                all.nextElement().clearSecondPhaseMark();
            }
        }
    ...
    private void build(Module mod, String[] mids) {
        ...
        if (doit >= 0) {
            ...
            String avoid = m_BuildOptions.getOption(Options.DROP_BOOKS);
            if (avoid.compareTo(Options.DROP_NONE) != 0) {
        ...
        if (doit >= 0) {
            try {
                ...
                m_Reporter.pushSimpleMessage("failed to build: " + mod.getName() + " because: " + ex);
                System.out.println("failed to build: " + mod.getName() + " because: " + ex);
            }
        }
    ...
    public void updateModules(HashMap<String, String> arguments) {
        ...
        // copy options from construction phase
        m_BuildOptions=new Options(m_ScriptOptions);
        if (arguments != null) {
    ...
    public Module getModuleById(String mid) {
        ...
        for (Module mod : m_RootModules) {
            @SuppressWarnings("unchecked")
            Enumeration<Module> mods = mod.depthFirstEnumeration();
            while (mods.hasMoreElements()) {
                Module m = mods.nextElement();
     ...
     */
    public final List<Module> getAllModules(String rootid) {
        ...
        // add the root and its subtree in preorder sequence
        @SuppressWarnings("unchecked")
        Enumeration<Module> all = root.preorderEnumeration();
        // include the root
        result.add(root);
        all.nextElement();
        while (all.hasMoreElements()) {
     ...
     * @param aslisted True if we want the modules as found
     * @return The list of modules
     */
    ...
    public String toString() {
        ...
        sb.append("\nScripthandler: "); 
        sb.append(m_scriptPath);
        sb.append("\n"); 
        sb.append(m_Def.toString());
    ...
    /**
     * Typically called from a GUI
     */
    public static String getOptionDescription(){
        return Options.getHTMLDescription();
    }
}
